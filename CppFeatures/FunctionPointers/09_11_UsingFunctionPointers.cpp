...
int count_if(std::vector<std::string>::iterator beg, const std::vector<std::string>::iterator &end, bool (*match)(std::string test))
{
    int count = 0;
    while (beg != end)
    {
...
int main()
{
    ...
    std::cout << "Local version: " << count_if(texts.begin(), texts.end(), match)
              << std::endl;
