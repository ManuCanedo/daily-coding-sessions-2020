...
int solution(std::vector<int>& A)
{
	...
	for (std::size_t i = 0; i < myRanges.size(); i++) {
		for (std::size_t j = i + 1; j < myRanges.size(); j++) {
			if (has_common_elements(myRanges[i].begin(), myRanges[i].end(),
						myRanges[j].begin(), myRanges[j].end())) {
				totalIntersections++;
...
template <class ForwardIt1, class ForwardIt2>
inline bool has_common_elements(ForwardIt1 first, ForwardIt1 last, ForwardIt2 s_first,
				ForwardIt2 s_last)
{
	auto it = first;
	auto s_it = s_first;
